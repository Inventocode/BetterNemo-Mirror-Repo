...
class BNGenerator {
    ...
    registerDefaultGenerators() {
        // 注册语句块生成器
        this.regStatement('start_on_click', (block) => {
            if (block.get_next_block())
                return `bnbc.start_on_click(() => {${this.nextBlocksToCode(block)}});`;
            return `bnbc.start_on_click(() => {});`;
        });
...
if (isPCTestEnv() && !PLAYER)
    (async () => {
        ...
        await sleep(100);
        document.addEventListener("mousedown", BetterNemo._.code);
