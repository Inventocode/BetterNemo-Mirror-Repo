.class public Lcom/codemao/nemo/activity/login/RegisterAction;
.super Lcom/codemao/nemo/activity/login/AccountLoginAction;
.source "RegisterAction.kt"


# direct methods
.method public static synthetic $r8$lambda$E9hhBE-n0Vi6VzzX-PSHwudy63Y(Lcom/codemao/nemo/activity/login/RegisterAction;Ljava/lang/String;)V
    .registers 2

    invoke-static {p0, p1}, Lcom/codemao/nemo/activity/login/RegisterAction;->getBindCode$lambda$3(Lcom/codemao/nemo/activity/login/RegisterAction;Ljava/lang/String;)V

    return-void
.end method

.method public static synthetic $r8$lambda$N-fBBewUiAUASetBiBVOBVNdq-g(Lcom/codemao/nemo/activity/login/RegisterAction;Ljava/lang/String;Ljava/lang/String;)V
    .registers 3

    invoke-static {p0, p1, p2}, Lcom/codemao/nemo/activity/login/RegisterAction;->register$lambda$2(Lcom/codemao/nemo/activity/login/RegisterAction;Ljava/lang/String;Ljava/lang/String;)V

    return-void
.end method

.method public static synthetic $r8$lambda$jyO_57ujgX2QhA18JE4frcF0Eoo(Lkotlin/jvm/functions/Function0;Lcom/codemao/nemo/activity/login/RegisterAction;Ljava/lang/String;Ljava/lang/String;)V
    .registers 4

    invoke-static {p0, p1, p2, p3}, Lcom/codemao/nemo/activity/login/RegisterAction;->getBindCode$lambda$4(Lkotlin/jvm/functions/Function0;Lcom/codemao/nemo/activity/login/RegisterAction;Ljava/lang/String;Ljava/lang/String;)V

    return-void
.end method

.method public static synthetic $r8$lambda$rYlWdVeojK4-FKi0gv0r5c2hJ1U(Lcom/codemao/nemo/activity/login/RegisterAction;Lcn/codemao/android/account/bean/SilentLoginResultVO;)V
    .registers 2

    invoke-static {p0, p1}, Lcom/codemao/nemo/activity/login/RegisterAction;->register$lambda$1(Lcom/codemao/nemo/activity/login/RegisterAction;Lcn/codemao/android/account/bean/SilentLoginResultVO;)V

    return-void
.end method

.method public constructor <init>()V
    .registers 1

    .line 39
    invoke-direct {p0}, Lcom/codemao/nemo/activity/login/AccountLoginAction;-><init>()V

    return-void
.end method

.method public static final synthetic access$setData(Lcom/codemao/nemo/activity/login/RegisterAction;J)V
    .registers 3

    .line 39
    invoke-direct {p0, p1, p2}, Lcom/codemao/nemo/activity/login/RegisterAction;->setData(J)V

    return-void
.end method

.method public static final synthetic access$toMain(Lcom/codemao/nemo/activity/login/RegisterAction;Z)V
    .registers 2

    .line 39
    invoke-direct {p0, p1}, Lcom/codemao/nemo/activity/login/RegisterAction;->toMain(Z)V

    return-void
.end method

.method private final fetchUserRegisterInfo(Z)V
    .registers 6

    .line 193
    invoke-static {}, Lcom/giu/xzz/http/RetrofitManager;->get()Lcom/giu/xzz/http/RetrofitManager;

    move-result-object v0

    const-class v1, Lcom/codemao/nemo/retrofit/api/UserService;

    invoke-virtual {v0, v1}, Lcom/giu/xzz/http/RetrofitManager;->create(Ljava/lang/Class;)Ljava/lang/Object;

    move-result-object v0

    check-cast v0, Lcom/codemao/nemo/retrofit/api/UserService;

    invoke-interface {v0}, Lcom/codemao/nemo/retrofit/api/UserService;->getUserRegisterInfo()Lio/reactivex/Observable;

    move-result-object v0

    .line 194
    sget-object v1, Ljava/util/concurrent/TimeUnit;->MILLISECONDS:Ljava/util/concurrent/TimeUnit;

    const-wide/16 v2, 0x7d0

    invoke-virtual {v0, v2, v3, v1}, Lio/reactivex/Observable;->timeout(JLjava/util/concurrent/TimeUnit;)Lio/reactivex/Observable;

    move-result-object v0

    .line 195
    invoke-static {}, Lcom/giu/xzz/http/transform/ThreadTransformer;->create()Lcom/giu/xzz/http/transform/ThreadTransformer;

    move-result-object v1

    invoke-virtual {v0, v1}, Lio/reactivex/Observable;->compose(Lio/reactivex/ObservableTransformer;)Lio/reactivex/Observable;

    move-result-object v0

    .line 197
    new-instance v1, Lcom/codemao/nemo/activity/login/RegisterAction$fetchUserRegisterInfo$1;

    invoke-direct {v1, p0, p1}, Lcom/codemao/nemo/activity/login/RegisterAction$fetchUserRegisterInfo$1;-><init>(Lcom/codemao/nemo/activity/login/RegisterAction;Z)V

    invoke-virtual {v0, v1}, Lio/reactivex/Observable;->subscribe(Lio/reactivex/Observer;)V

    return-void
.end method

.method private final getBindCode(Lkotlin/jvm/functions/Function0;)V
    .registers 6
    .annotation system Ldalvik/annotation/Signature;
        value = {
            "(",
            "Lkotlin/jvm/functions/Function0<",
            "Lkotlin/Unit;",
            ">;)V"
        }
    .end annotation

    .line 255
    invoke-virtual {p0}, Lcom/codemao/nemo/activity/login/AccountLoginAction;->getFirstLoginLayout()Lcom/codemao/nemo/view/LoginInputLayout;

    move-result-object v0

    if-eqz v0, :cond_3d

    invoke-virtual {v0}, Lcom/codemao/nemo/view/LoginInputLayout;->getEditTextStr()Ljava/lang/String;

    move-result-object v0

    if-nez v0, :cond_d

    goto :goto_3d

    .line 256
    :cond_d
    invoke-virtual {p0}, Lcom/codemao/nemo/activity/login/AccountLoginAction;->getContext()Lcom/codemao/nemo/activity/login/AccountLoginActivityV3;

    move-result-object v1

    invoke-static {v1}, Lcom/giu/xzz/utils/NetworkHelper;->isConnect(Landroid/content/Context;)Z

    move-result v1

    if-nez v1, :cond_1f

    .line 257
    invoke-virtual {p0}, Lcom/codemao/nemo/activity/login/AccountLoginAction;->getContext()Lcom/codemao/nemo/activity/login/AccountLoginActivityV3;

    move-result-object p1

    invoke-static {p1}, Lcom/codemao/nemo/util/ToastUtil;->showNetErrorToast(Landroid/content/Context;)V

    return-void

    .line 260
    :cond_1f
    invoke-static {v0}, Lcom/codemao/nemo/util/StringUtil;->isPhoneNum(Ljava/lang/String;)Z

    move-result v1

    if-nez v1, :cond_2c

    const-string/jumbo p1, "请输入正确的手机号"

    .line 261
    invoke-virtual {p0, p1}, Lcom/codemao/nemo/activity/login/AccountLoginAction;->showDialogDelay(Ljava/lang/String;)V

    return-void

    .line 264
    :cond_2c
    invoke-virtual {p0}, Lcom/codemao/nemo/activity/login/AccountLoginAction;->getContext()Lcom/codemao/nemo/activity/login/AccountLoginActivityV3;

    move-result-object v1

    new-instance v2, Lcom/codemao/nemo/activity/login/RegisterAction$$ExternalSyntheticLambda3;

    invoke-direct {v2, p0}, Lcom/codemao/nemo/activity/login/RegisterAction$$ExternalSyntheticLambda3;-><init>(Lcom/codemao/nemo/activity/login/RegisterAction;)V

    new-instance v3, Lcom/codemao/nemo/activity/login/RegisterAction$$ExternalSyntheticLambda1;

    invoke-direct {v3, p1, p0}, Lcom/codemao/nemo/activity/login/RegisterAction$$ExternalSyntheticLambda1;-><init>(Lkotlin/jvm/functions/Function0;Lcom/codemao/nemo/activity/login/RegisterAction;)V

    invoke-static {v1, v0, v2, v3}, Lcn/codemao/android/account/CodeMaoAccount;->getSilentLoginCapcha(Landroid/content/Context;Ljava/lang/String;Lcn/codemao/android/account/listener/NetSuccessResultListener;Lcn/codemao/android/account/listener/NetFailResultListener;)V

    :cond_3d
    :goto_3d
    return-void
.end method

.method private static final getBindCode$lambda$3(Lcom/codemao/nemo/activity/login/RegisterAction;Ljava/lang/String;)V
    .registers 2

    const-string p1, "this$0"

    invoke-static {p0, p1}, Lkotlin/jvm/internal/Intrinsics;->checkNotNullParameter(Ljava/lang/Object;Ljava/lang/String;)V

    const-string/jumbo p1, "验证码已发送"

    .line 264
    invoke-virtual {p0, p1}, Lcom/codemao/nemo/activity/login/AccountLoginAction;->showDialogDelayWithCheckmark(Ljava/lang/String;)V

    return-void
.end method

.method private static final getBindCode$lambda$4(Lkotlin/jvm/functions/Function0;Lcom/codemao/nemo/activity/login/RegisterAction;Ljava/lang/String;Ljava/lang/String;)V
    .registers 4

    const-string p3, "$onCancel"

    invoke-static {p0, p3}, Lkotlin/jvm/internal/Intrinsics;->checkNotNullParameter(Ljava/lang/Object;Ljava/lang/String;)V

    const-string p3, "this$0"

    invoke-static {p1, p3}, Lkotlin/jvm/internal/Intrinsics;->checkNotNullParameter(Ljava/lang/Object;Ljava/lang/String;)V

    .line 265
    invoke-interface {p0}, Lkotlin/jvm/functions/Function0;->invoke()Ljava/lang/Object;

    const-string p0, "A_5"

    .line 267
    invoke-static {p2, p0}, Lkotlin/jvm/internal/Intrinsics;->areEqual(Ljava/lang/Object;Ljava/lang/Object;)Z

    move-result p0

    if-eqz p0, :cond_1c

    const-string/jumbo p0, "请输入正确的手机号"

    invoke-virtual {p1, p0}, Lcom/codemao/nemo/activity/login/AccountLoginAction;->showDialogDelay(Ljava/lang/String;)V

    goto :goto_22

    :cond_1c
    const-string/jumbo p0, "获取验证码失败，请重新尝试"

    .line 268
    invoke-virtual {p1, p0}, Lcom/codemao/nemo/activity/login/AccountLoginAction;->showDialogDelay(Ljava/lang/String;)V

    :goto_22
    return-void
.end method

.method private final register()V
    .registers 6

    .line 157
    invoke-virtual {p0}, Lcom/codemao/nemo/activity/login/AccountLoginAction;->getContext()Lcom/codemao/nemo/activity/login/AccountLoginActivityV3;

    move-result-object v0

    invoke-static {v0}, Lcom/giu/xzz/utils/NetworkHelper;->isConnect(Landroid/content/Context;)Z

    move-result v0

    if-nez v0, :cond_12

    .line 158
    invoke-virtual {p0}, Lcom/codemao/nemo/activity/login/AccountLoginAction;->getContext()Lcom/codemao/nemo/activity/login/AccountLoginActivityV3;

    move-result-object v0

    invoke-static {v0}, Lcom/codemao/nemo/util/ToastUtil;->showNetErrorToast(Landroid/content/Context;)V

    return-void

    .line 161
    :cond_12
    invoke-virtual {p0}, Lcom/codemao/nemo/activity/login/AccountLoginAction;->getFirstLoginLayout()Lcom/codemao/nemo/view/LoginInputLayout;

    move-result-object v0

    if-eqz v0, :cond_43

    invoke-virtual {v0}, Lcom/codemao/nemo/view/LoginInputLayout;->getEditTextStr()Ljava/lang/String;

    move-result-object v0

    if-nez v0, :cond_1f

    goto :goto_43

    .line 162
    :cond_1f
    invoke-virtual {p0}, Lcom/codemao/nemo/activity/login/AccountLoginAction;->getSecondLoginLayout()Lcom/codemao/nemo/view/LoginInputLayout;

    move-result-object v1

    if-eqz v1, :cond_43

    invoke-virtual {v1}, Lcom/codemao/nemo/view/LoginInputLayout;->getEditTextStr()Ljava/lang/String;

    move-result-object v1

    if-nez v1, :cond_2c

    goto :goto_43

    :cond_2c
    const-string/jumbo v2, "正在注册..."

    .line 163
    invoke-virtual {p0, v2}, Lcom/codemao/nemo/activity/login/AccountLoginAction;->showDialog(Ljava/lang/String;)V

    .line 164
    invoke-static {}, Lcom/codemao/nemo/util/LocalUserHelper;->getCurUserAgreeIds()Ljava/util/List;

    move-result-object v2

    new-instance v3, Lcom/codemao/nemo/activity/login/RegisterAction$$ExternalSyntheticLambda2;

    invoke-direct {v3, p0}, Lcom/codemao/nemo/activity/login/RegisterAction$$ExternalSyntheticLambda2;-><init>(Lcom/codemao/nemo/activity/login/RegisterAction;)V

    new-instance v4, Lcom/codemao/nemo/activity/login/RegisterAction$$ExternalSyntheticLambda0;

    invoke-direct {v4, p0}, Lcom/codemao/nemo/activity/login/RegisterAction$$ExternalSyntheticLambda0;-><init>(Lcom/codemao/nemo/activity/login/RegisterAction;)V

    invoke-static {v0, v1, v2, v3, v4}, Lcn/codemao/android/account/CodeMaoAccount;->silentLogin(Ljava/lang/String;Ljava/lang/String;Ljava/util/List;Lcn/codemao/android/account/listener/NetSuccessResultListener;Lcn/codemao/android/account/listener/NetFailResultListener;)V

    :cond_43
    :goto_43
    return-void
.end method

.method private static final register$lambda$1(Lcom/codemao/nemo/activity/login/RegisterAction;Lcn/codemao/android/account/bean/SilentLoginResultVO;)V
    .registers 5

    const-string v0, "this$0"

    invoke-static {p0, v0}, Lkotlin/jvm/internal/Intrinsics;->checkNotNullParameter(Ljava/lang/Object;Ljava/lang/String;)V

    .line 165
    invoke-virtual {p0}, Lcom/codemao/nemo/activity/login/AccountLoginAction;->hideDialog()V

    .line 166
    invoke-virtual {p1}, Lcn/codemao/android/account/bean/SilentLoginResultVO;->isIs_first_login()Z

    move-result v0

    sput-boolean v0, Lcom/codemao/nemo/util/AppConstants;->isFirstLogin:Z

    .line 167
    invoke-virtual {p1}, Lcn/codemao/android/account/bean/SilentLoginResultVO;->isIs_first_login()Z

    move-result v0

    const-string v1, "4"

    const/4 v2, 0x0

    if-nez v0, :cond_1e

    .line 168
    invoke-static {v1, v2}, Lcom/codemao/nemo/sdk/stat/StatHelper;->sendLoginEvent(Ljava/lang/String;Z)V

    .line 169
    invoke-direct {p0, v2}, Lcom/codemao/nemo/activity/login/RegisterAction;->fetchUserRegisterInfo(Z)V

    goto :goto_41

    .line 171
    :cond_1e
    invoke-virtual {p1}, Lcn/codemao/android/account/bean/LoginUserInfoVO;->getAuth()Lcn/codemao/android/account/bean/LoginUserInfoVO$AuthBean;

    move-result-object p1

    invoke-virtual {p1}, Lcn/codemao/android/account/bean/LoginUserInfoVO$AuthBean;->isHas_password()Z

    move-result p1

    if-eqz p1, :cond_30

    const/4 p1, 0x1

    .line 172
    invoke-static {v1, p1}, Lcom/codemao/nemo/sdk/stat/StatHelper;->sendLoginEvent(Ljava/lang/String;Z)V

    .line 173
    invoke-direct {p0, p1}, Lcom/codemao/nemo/activity/login/RegisterAction;->fetchUserRegisterInfo(Z)V

    goto :goto_41

    .line 175
    :cond_30
    invoke-virtual {p0}, Lcom/codemao/nemo/activity/login/AccountLoginAction;->getContext()Lcom/codemao/nemo/activity/login/AccountLoginActivityV3;

    move-result-object p0

    if-eqz p0, :cond_41

    new-instance p1, Lcom/codemao/nemo/activity/login/SetPasswordAction;

    invoke-direct {p1, v2}, Lcom/codemao/nemo/activity/login/SetPasswordAction;-><init>(I)V

    const-string/jumbo v0, "新用户注册设置密码页"

    invoke-virtual {p0, p1, v0}, Lcom/codemao/nemo/activity/login/AccountLoginActivityV3;->changePageAction(Lcom/codemao/nemo/activity/login/AccountLoginAction;Ljava/lang/String;)V

    :cond_41
    :goto_41
    return-void
.end method

.method private static final register$lambda$2(Lcom/codemao/nemo/activity/login/RegisterAction;Ljava/lang/String;Ljava/lang/String;)V
    .registers 6

    const-string p2, "this$0"

    invoke-static {p0, p2}, Lkotlin/jvm/internal/Intrinsics;->checkNotNullParameter(Ljava/lang/Object;Ljava/lang/String;)V

    .line 179
    invoke-virtual {p0}, Lcom/codemao/nemo/activity/login/AccountLoginAction;->hideDialog()V

    const-string/jumbo p2, "请输入正确的手机号"

    if-eqz p1, :cond_4b

    .line 180
    invoke-virtual {p1}, Ljava/lang/String;->hashCode()I

    move-result v0

    const v1, 0xfc51

    const-string/jumbo v2, "验证码输入错误"

    if-eq v0, v1, :cond_3e

    const v1, 0xffb7

    if-eq v0, v1, :cond_31

    const v1, 0x72b3c0d0

    if-eq v0, v1, :cond_24

    goto :goto_4b

    :cond_24
    const-string v0, "AC3_13"

    invoke-virtual {p1, v0}, Ljava/lang/String;->equals(Ljava/lang/Object;)Z

    move-result p1

    if-nez p1, :cond_2d

    goto :goto_4b

    .line 181
    :cond_2d
    invoke-virtual {p0, v2}, Lcom/codemao/nemo/activity/login/AccountLoginAction;->showDialogDelay(Ljava/lang/String;)V

    goto :goto_4e

    :cond_31
    const-string v0, "A_5"

    .line 180
    invoke-virtual {p1, v0}, Ljava/lang/String;->equals(Ljava/lang/Object;)Z

    move-result p1

    if-nez p1, :cond_3a

    goto :goto_4b

    .line 183
    :cond_3a
    invoke-virtual {p0, p2}, Lcom/codemao/nemo/activity/login/AccountLoginAction;->showDialogDelay(Ljava/lang/String;)V

    goto :goto_4e

    :cond_3e
    const-string v0, "AC3"

    .line 180
    invoke-virtual {p1, v0}, Ljava/lang/String;->equals(Ljava/lang/Object;)Z

    move-result p1

    if-nez p1, :cond_47

    goto :goto_4b

    .line 182
    :cond_47
    invoke-virtual {p0, v2}, Lcom/codemao/nemo/activity/login/AccountLoginAction;->showDialogDelay(Ljava/lang/String;)V

    goto :goto_4e

    .line 184
    :cond_4b
    :goto_4b
    invoke-virtual {p0, p2}, Lcom/codemao/nemo/activity/login/AccountLoginAction;->showDialogDelay(Ljava/lang/String;)V

    :goto_4e
    return-void
.end method

.method private final setCookie(Ljava/lang/String;)V
    .registers 8

    .line 235
    invoke-virtual {p0}, Lcom/codemao/nemo/activity/login/AccountLoginAction;->getContext()Lcom/codemao/nemo/activity/login/AccountLoginActivityV3;

    move-result-object v0

    invoke-static {v0}, Landroid/webkit/CookieSyncManager;->createInstance(Landroid/content/Context;)Landroid/webkit/CookieSyncManager;

    move-result-object v0

    invoke-virtual {v0}, Landroid/webkit/CookieSyncManager;->startSync()V

    .line 236
    invoke-static {}, Landroid/webkit/CookieManager;->getInstance()Landroid/webkit/CookieManager;

    move-result-object v0

    const/4 v1, 0x1

    .line 237
    invoke-virtual {v0, v1}, Landroid/webkit/CookieManager;->setAcceptCookie(Z)V

    .line 238
    invoke-virtual {v0}, Landroid/webkit/CookieManager;->removeSessionCookie()V

    .line 239
    invoke-virtual {v0}, Landroid/webkit/CookieManager;->removeAllCookie()V

    .line 240
    sget-object v1, Landroid/os/Build;->MODEL:Ljava/lang/String;

    const-string v2, "MODEL"

    invoke-static {v1, v2}, Lkotlin/jvm/internal/Intrinsics;->checkNotNullExpressionValue(Ljava/lang/Object;Ljava/lang/String;)V

    const-string v2, "BBL-"

    const/4 v3, 0x0

    const/4 v4, 0x2

    const/4 v5, 0x0

    invoke-static {v1, v2, v3, v4, v5}, Lkotlin/text/StringsKt;->startsWith$default(Ljava/lang/String;Ljava/lang/String;ZILjava/lang/Object;)Z

    move-result v1

    if-nez v1, :cond_34

    .line 241
    invoke-static {}, Lcn/codemao/android/account/CodeMaoAccount;->getAccountToken()Ljava/lang/String;

    move-result-object p1

    const-string v1, "getAccountToken()"

    invoke-static {p1, v1}, Lkotlin/jvm/internal/Intrinsics;->checkNotNullExpressionValue(Ljava/lang/Object;Ljava/lang/String;)V

    .line 243
    :cond_34
    sget-object v1, Lcom/codemao/nemo/MainApplication;->configVO:Lcom/codemao/nemo/bean/ConfigVO;

    invoke-virtual {v1}, Lcom/codemao/nemo/bean/ConfigVO;->getApi()Lcom/codemao/nemo/bean/ApiVO;

    move-result-object v1

    invoke-virtual {v1}, Lcom/codemao/nemo/bean/ApiVO;->getHost()Ljava/lang/String;

    move-result-object v1

    .line 244
    sget-boolean v2, Lcom/codemao/nemo/util/AppConstants;->IS_DEBUG:Z

    if-eqz v2, :cond_57

    .line 245
    new-instance v2, Ljava/lang/StringBuilder;

    invoke-direct {v2}, Ljava/lang/StringBuilder;-><init>()V

    const-string v3, "test-authorization="

    invoke-virtual {v2, v3}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    invoke-virtual {v2, p1}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    invoke-virtual {v2}, Ljava/lang/StringBuilder;->toString()Ljava/lang/String;

    move-result-object p1

    invoke-virtual {v0, v1, p1}, Landroid/webkit/CookieManager;->setCookie(Ljava/lang/String;Ljava/lang/String;)V

    goto :goto_84

    .line 246
    :cond_57
    sget-boolean v2, Lcom/codemao/nemo/util/AppConstants;->IS_RELEASE:Z

    if-eqz v2, :cond_70

    .line 247
    new-instance v2, Ljava/lang/StringBuilder;

    invoke-direct {v2}, Ljava/lang/StringBuilder;-><init>()V

    const-string v3, "authorization="

    invoke-virtual {v2, v3}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    invoke-virtual {v2, p1}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    invoke-virtual {v2}, Ljava/lang/StringBuilder;->toString()Ljava/lang/String;

    move-result-object p1

    invoke-virtual {v0, v1, p1}, Landroid/webkit/CookieManager;->setCookie(Ljava/lang/String;Ljava/lang/String;)V

    goto :goto_84

    .line 249
    :cond_70
    new-instance v2, Ljava/lang/StringBuilder;

    invoke-direct {v2}, Ljava/lang/StringBuilder;-><init>()V

    const-string v3, "staging-authorization="

    invoke-virtual {v2, v3}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    invoke-virtual {v2, p1}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    invoke-virtual {v2}, Ljava/lang/StringBuilder;->toString()Ljava/lang/String;

    move-result-object p1

    invoke-virtual {v0, v1, p1}, Landroid/webkit/CookieManager;->setCookie(Ljava/lang/String;Ljava/lang/String;)V

    .line 251
    :goto_84
    invoke-static {}, Landroid/webkit/CookieSyncManager;->getInstance()Landroid/webkit/CookieSyncManager;

    move-result-object p1

    invoke-virtual {p1}, Landroid/webkit/CookieSyncManager;->sync()V

    return-void
.end method

.method private final setData(J)V
    .registers 5

    .line 223
    invoke-static {}, Lcn/codemao/android/account/CodeMaoAccount;->getCachedUserInfo()Lcn/codemao/android/account/bean/UserInfoVO;

    move-result-object v0

    .line 224
    invoke-static {}, Lcom/codemao/nemo/util/BcmHelper;->initPath()V

    .line 225
    new-instance v1, Lcom/codemao/nemo/bean/LocalUserInfo;

    invoke-direct {v1}, Lcom/codemao/nemo/bean/LocalUserInfo;-><init>()V

    .line 226
    invoke-virtual {v1, v0}, Lcom/codemao/nemo/bean/LocalUserInfo;->clone(Lcn/codemao/android/account/bean/UserInfoVO;)V

    const/4 v0, 0x0

    .line 227
    invoke-static {v0}, Lcom/codemao/nemo/util/LocalUserHelper;->updateIsBBKLogin(Z)V

    .line 229
    invoke-static {v1, p1, p2}, Lcom/codemao/nemo/util/LocalUserHelper;->updateUserInfo(Lcom/codemao/nemo/bean/LocalUserInfo;J)V

    .line 230
    invoke-static {}, Lcn/codemao/android/account/CodeMaoAccount;->getAccountToken()Ljava/lang/String;

    move-result-object p1

    const-string p2, "getAccountToken()"

    invoke-static {p1, p2}, Lkotlin/jvm/internal/Intrinsics;->checkNotNullExpressionValue(Ljava/lang/Object;Ljava/lang/String;)V

    invoke-direct {p0, p1}, Lcom/codemao/nemo/activity/login/RegisterAction;->setCookie(Ljava/lang/String;)V

    return-void
.end method

.method private final toMain(Z)V
    .registers 4

    .line 274
    sget v0, Lcom/codemao/nemo/event/LoginEvent;->type:I

    const/4 v1, 0x2

    if-ne v0, v1, :cond_c

    .line 275
    invoke-virtual {p0}, Lcom/codemao/nemo/activity/login/AccountLoginAction;->getContext()Lcom/codemao/nemo/activity/login/AccountLoginActivityV3;

    move-result-object v0

    invoke-static {v0, p1}, Lcom/codemao/nemo/MainActivityV2;->startMainActivityV2(Landroid/content/Context;Z)V

    .line 277
    :cond_c
    invoke-static {}, Lcom/codemao/nemo/util/ActivityUtils;->get()Lcom/codemao/nemo/util/ActivityUtils;

    move-result-object p1

    const-class v0, Lcom/codemao/nemo/activity/LoginActivity;

    invoke-virtual {p1, v0}, Lcom/codemao/nemo/util/ActivityUtils;->killActivity(Ljava/lang/Class;)V

    .line 278
    invoke-static {}, Lcom/codemao/nemo/util/ActivityUtils;->get()Lcom/codemao/nemo/util/ActivityUtils;

    move-result-object p1

    const-class v0, Lcom/codemao/nemo/activity/login/AccountLoginActivityV3;

    invoke-virtual {p1, v0}, Lcom/codemao/nemo/util/ActivityUtils;->killActivity(Ljava/lang/Class;)V

    return-void
.end method


# virtual methods
.method public onBackPressed()Z
    .registers 2

    const/4 v0, 0x1

    return v0
.end method

.method public onConfigurationChanged(Landroid/content/res/Configuration;)V
    .registers 3

    const-string v0, "newConfig"

    invoke-static {p1, v0}, Lkotlin/jvm/internal/Intrinsics;->checkNotNullParameter(Ljava/lang/Object;Ljava/lang/String;)V

    .line 128
    invoke-super {p0, p1}, Lcom/codemao/nemo/activity/login/AccountLoginAction;->onConfigurationChanged(Landroid/content/res/Configuration;)V

    .line 129
    invoke-static {p1}, Lcn/codemao/android/account/CodeMaoAccount;->onConfigurationChanged(Landroid/content/res/Configuration;)V

    return-void
.end method

.method public onConfirmClick()V
    .registers 5

    .line 145
    sget-object v0, Lcom/codemao/nemo/evolving/DGMLog;->INSTANCE:Lcom/codemao/nemo/evolving/DGMLog;

    const-string/jumbo v1, "点击注册"

    invoke-virtual {v0, v1}, Lcom/codemao/nemo/evolving/DGMLog;->dLogin(Ljava/lang/String;)V

    .line 146
    invoke-virtual {p0}, Lcom/codemao/nemo/activity/login/AccountLoginAction;->getSecondLoginLayout()Lcom/codemao/nemo/view/LoginInputLayout;

    move-result-object v0

    invoke-static {v0}, Lcom/codemao/nemo/util/SoftInputUtils;->hideSoftInput(Landroid/view/View;)V

    .line 147
    new-instance v0, Ljava/lang/StringBuilder;

    invoke-direct {v0}, Ljava/lang/StringBuilder;-><init>()V

    invoke-virtual {p0}, Lcom/codemao/nemo/activity/login/AccountLoginAction;->getViewName()Ljava/lang/String;

    move-result-object v1

    invoke-virtual {v0, v1}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    const-string v1, "-注册"

    invoke-virtual {v0, v1}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    invoke-virtual {v0}, Ljava/lang/StringBuilder;->toString()Ljava/lang/String;

    move-result-object v0

    const/4 v1, 0x1

    new-array v1, v1, [Ljava/lang/String;

    invoke-static {}, Lcom/codemao/nemo/util/SystemUtil;->getChannel()Ljava/lang/String;

    move-result-object v2

    const/4 v3, 0x0

    aput-object v2, v1, v3

    invoke-static {v0, v1}, Lcom/codemao/nemo/sdk/stat/StatHelper;->sendNewReportEvent(Ljava/lang/String;[Ljava/lang/String;)V

    const-string/jumbo v0, "注册"

    .line 148
    invoke-virtual {p0, v0}, Lcom/codemao/nemo/activity/login/AccountLoginAction;->reportEvent(Ljava/lang/String;)V

    .line 149
    invoke-direct {p0}, Lcom/codemao/nemo/activity/login/RegisterAction;->register()V

    return-void
.end method

.method public onDestroy()V
    .registers 2

    .line 133
    invoke-super {p0}, Lcom/codemao/nemo/activity/login/AccountLoginAction;->onDestroy()V

    .line 134
    invoke-static {}, Lorg/greenrobot/eventbus/EventBus;->getDefault()Lorg/greenrobot/eventbus/EventBus;

    move-result-object v0

    invoke-virtual {v0, p0}, Lorg/greenrobot/eventbus/EventBus;->unregister(Ljava/lang/Object;)V

    .line 135
    invoke-static {}, Lcn/codemao/android/account/CodeMaoAccount;->onDestroy()V

    return-void
.end method

.method public onGetCaptchaClick(Lkotlin/jvm/functions/Function0;)V
    .registers 6
    .annotation system Ldalvik/annotation/Signature;
        value = {
            "(",
            "Lkotlin/jvm/functions/Function0<",
            "Lkotlin/Unit;",
            ">;)V"
        }
    .end annotation

    const-string v0, "onCancel"

    invoke-static {p1, v0}, Lkotlin/jvm/internal/Intrinsics;->checkNotNullParameter(Ljava/lang/Object;Ljava/lang/String;)V

    .line 139
    new-instance v0, Ljava/lang/StringBuilder;

    invoke-direct {v0}, Ljava/lang/StringBuilder;-><init>()V

    invoke-virtual {p0}, Lcom/codemao/nemo/activity/login/AccountLoginAction;->getViewName()Ljava/lang/String;

    move-result-object v1

    invoke-virtual {v0, v1}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    const-string v1, "-获取验证码"

    invoke-virtual {v0, v1}, Ljava/lang/StringBuilder;->append(Ljava/lang/String;)Ljava/lang/StringBuilder;

    invoke-virtual {v0}, Ljava/lang/StringBuilder;->toString()Ljava/lang/String;

    move-result-object v0

    const/4 v1, 0x1

    new-array v1, v1, [Ljava/lang/String;

    invoke-static {}, Lcom/codemao/nemo/util/SystemUtil;->getChannel()Ljava/lang/String;

    move-result-object v2

    const/4 v3, 0x0

    aput-object v2, v1, v3

    invoke-static {v0, v1}, Lcom/codemao/nemo/sdk/stat/StatHelper;->sendNewReportEvent(Ljava/lang/String;[Ljava/lang/String;)V

    const-string/jumbo v0, "获取验证码"

    .line 140
    invoke-virtual {p0, v0}, Lcom/codemao/nemo/activity/login/AccountLoginAction;->reportEvent(Ljava/lang/String;)V

    .line 141
    invoke-direct {p0, p1}, Lcom/codemao/nemo/activity/login/RegisterAction;->getBindCode(Lkotlin/jvm/functions/Function0;)V

    return-void
.end method

.method public onInit()V
    .registers 5

    .line 42
    invoke-static {}, Lorg/greenrobot/eventbus/EventBus;->getDefault()Lorg/greenrobot/eventbus/EventBus;

    move-result-object v0

    invoke-virtual {v0, p0}, Lorg/greenrobot/eventbus/EventBus;->register(Ljava/lang/Object;)V

    .line 43
    invoke-virtual {p0}, Lcom/codemao/nemo/activity/login/AccountLoginAction;->getContext()Lcom/codemao/nemo/activity/login/AccountLoginActivityV3;

    move-result-object v0

    invoke-static {v0}, Lcn/codemao/android/account/CodeMaoAccount;->onCreate(Landroid/content/Context;)V

    .line 45
    invoke-virtual {p0}, Lcom/codemao/nemo/activity/login/AccountLoginAction;->getTitleTextView()Landroid/widget/TextView;

    move-result-object v0

    if-nez v0, :cond_15

    goto :goto_1b

    :cond_15
    const-string/jumbo v1, "新用户注册"

    invoke-virtual {v0, v1}, Landroid/widget/TextView;->setText(Ljava/lang/CharSequence;)V

    .line 46
    :goto_1b
    invoke-virtual {p0}, Lcom/codemao/nemo/activity/login/AccountLoginAction;->getTitleTextView()Landroid/widget/TextView;

    move-result-object v0

    const/4 v1, 0x1

    if-eqz v0, :cond_27

    const/high16 v2, 0x41b00000  # 22.0f

    invoke-virtual {v0, v1, v2}, Landroid/widget/TextView;->setTextSize(IF)V

    .line 47
    :cond_27
    invoke-virtual {p0}, Lcom/codemao/nemo/activity/login/AccountLoginAction;->getContext()Lcom/codemao/nemo/activity/login/AccountLoginActivityV3;

    move-result-object v0

    if-eqz v0, :cond_41

    .line 48
    invoke-virtual {p0}, Lcom/codemao/nemo/activity/login/AccountLoginAction;->getTitleTextView()Landroid/widget/TextView;

    move-result-object v2

    if-eqz v2, :cond_41

    invoke-virtual {v0}, Landroidx/appcompat/app/AppCompatActivity;->getResources()Landroid/content/res/Resources;

    move-result-object v0

    const v3, 0x7f0601af

    invoke-virtual {v0, v3}, Landroid/content/res/Resources;->getColor(I)I

    move-result v0

    invoke-virtual {v2, v0}, Landroid/widget/TextView;->setTextColor(I)V

    .line 50
    :cond_41
    invoke-virtual {p0}, Lcom/codemao/nemo/activity/login/AccountLoginAction;->getFirstLoginLayout()Lcom/codemao/nemo/view/LoginInputLayout;

    move-result-object v0

    if-eqz v0, :cond_4d

    const v2, 0x7f0e00f7

    invoke-virtual {v0, v2}, Lcom/codemao/nemo/view/LoginInputLayout;->setFirstIconRes(I)V

    .line 51
    :cond_4d
    invoke-virtual {p0}, Lcom/codemao/nemo/activity/login/AccountLoginAction;->getFirstLoginLayout()Lcom/codemao/nemo/view/LoginInputLayout;

    move-result-object v0

    if-eqz v0, :cond_59

    const-string/jumbo v2, "手机号"

    invoke-virtual {v0, v2}, Lcom/codemao/nemo/view/LoginInputLayout;->setEditTextHint(Ljava/lang/String;)V

    .line 52
    :cond_59
    invoke-virtual {p0}, Lcom/codemao/nemo/activity/login/AccountLoginAction;->getFirstLoginLayout()Lcom/codemao/nemo/view/LoginInputLayout;

    move-result-object v0

    if-eqz v0, :cond_62

    invoke-virtual {v0, v1}, Lcom/codemao/nemo/view/LoginInputLayout;->setInputType(I)V

    .line 53
    :cond_62
    invoke-virtual {p0}, Lcom/codemao/nemo/activity/login/AccountLoginAction;->getFirstLoginLayout()Lcom/codemao/nemo/view/LoginInputLayout;

    move-result-object v0

    if-eqz v0, :cond_6d

    const/16 v2, 0xb

    invoke-virtual {v0, v2}, Lcom/codemao/nemo/view/LoginInputLayout;->setEditTextMaxLength(I)V

    .line 54
    :cond_6d
    invoke-virtual {p0}, Lcom/codemao/nemo/activity/login/AccountLoginAction;->getFirstLoginLayout()Lcom/codemao/nemo/view/LoginInputLayout;

    move-result-object v0

    const/4 v2, 0x0

    if-eqz v0, :cond_77

    invoke-virtual {v0, v2}, Lcom/codemao/nemo/view/LoginInputLayout;->setConfirmTextViewVisible(Z)V

    .line 56
    :cond_77
    invoke-virtual {p0}, Lcom/codemao/nemo/activity/login/AccountLoginAction;->getSecondLoginLayout()Lcom/codemao/nemo/view/LoginInputLayout;

    move-result-object v0

    if-eqz v0, :cond_83

    const v3, 0x7f0e00f4

    invoke-virtual {v0, v3}, Lcom/codemao/nemo/view/LoginInputLayout;->setFirstIconRes(I)V

    .line 57
    :cond_83
    invoke-virtual {p0}, Lcom/codemao/nemo/activity/login/AccountLoginAction;->getSecondLoginLayout()Lcom/codemao/nemo/view/LoginInputLayout;

    move-result-object v0

    if-eqz v0, :cond_8f

    const-string/jumbo v3, "验证码"

    invoke-virtual {v0, v3}, Lcom/codemao/nemo/view/LoginInputLayout;->setEditTextHint(Ljava/lang/String;)V

    .line 58
    :cond_8f
    invoke-virtual {p0}, Lcom/codemao/nemo/activity/login/AccountLoginAction;->getSecondLoginLayout()Lcom/codemao/nemo/view/LoginInputLayout;

    move-result-object v0

    if-eqz v0, :cond_98

    invoke-virtual {v0, v1}, Lcom/codemao/nemo/view/LoginInputLayout;->setInputType(I)V

    .line 59
    :cond_98
    invoke-virtual {p0}, Lcom/codemao/nemo/activity/login/AccountLoginAction;->getSecondLoginLayout()Lcom/codemao/nemo/view/LoginInputLayout;

    move-result-object v0

    if-eqz v0, :cond_a2

    const/4 v3, 0x6

    invoke-virtual {v0, v3}, Lcom/codemao/nemo/view/LoginInputLayout;->setEditTextMaxLength(I)V

    .line 60
    :cond_a2
    invoke-virtual {p0}, Lcom/codemao/nemo/activity/login/AccountLoginAction;->getSecondLoginLayout()Lcom/codemao/nemo/view/LoginInputLayout;

    move-result-object v0

    if-eqz v0, :cond_ab

    invoke-virtual {v0, v1}, Lcom/codemao/nemo/view/LoginInputLayout;->setConfirmTextViewVisible(Z)V

    .line 62
    :cond_ab
    invoke-virtual {p0}, Lcom/codemao/nemo/activity/login/AccountLoginAction;->getBottomLeftTextView()Landroid/widget/TextView;

    move-result-object v0

    const/16 v1, 0x8

    if-nez v0, :cond_b4

    goto :goto_b7

    :cond_b4
    invoke-virtual {v0, v1}, Landroid/widget/TextView;->setVisibility(I)V

    .line 63
    :goto_b7
    invoke-virtual {p0}, Lcom/codemao/nemo/activity/login/AccountLoginAction;->getBottomRightTextView()Landroid/widget/TextView;

    move-result-object v0

    if-nez v0, :cond_be

    goto :goto_c1

    :cond_be
    invoke-virtual {v0, v1}, Landroid/widget/TextView;->setVisibility(I)V

    .line 65
    :goto_c1
    invoke-virtual {p0}, Lcom/codemao/nemo/activity/login/AccountLoginAction;->getBottomMiddleTextView()Landroid/widget/TextView;

    move-result-object v0

    if-nez v0, :cond_c8

    goto :goto_cb

    :cond_c8
    invoke-virtual {v0, v1}, Landroid/widget/TextView;->setVisibility(I)V

    .line 101
    :goto_cb
    invoke-virtual {p0}, Lcom/codemao/nemo/activity/login/AccountLoginAction;->getConfirmBtn()Landroid/widget/TextView;

    move-result-object v0

    if-nez v0, :cond_d2

    goto :goto_d8

    :cond_d2
    const-string/jumbo v1, "注 册"

    invoke-virtual {v0, v1}, Landroid/widget/TextView;->setText(Ljava/lang/CharSequence;)V

    .line 102
    :goto_d8
    invoke-virtual {p0}, Lcom/codemao/nemo/activity/login/AccountLoginAction;->getConfirmBtn()Landroid/widget/TextView;

    move-result-object v0

    if-nez v0, :cond_df

    goto :goto_e2

    :cond_df
    invoke-virtual {v0, v2}, Landroid/widget/TextView;->setEnabled(Z)V

    .line 104
    :goto_e2
    new-instance v0, Lcom/codemao/nemo/activity/login/RegisterAction$onInit$textWatcher$1;

    invoke-direct {v0, p0}, Lcom/codemao/nemo/activity/login/RegisterAction$onInit$textWatcher$1;-><init>(Lcom/codemao/nemo/activity/login/RegisterAction;)V

    .line 123
    invoke-virtual {p0}, Lcom/codemao/nemo/activity/login/AccountLoginAction;->getFirstLoginLayout()Lcom/codemao/nemo/view/LoginInputLayout;

    move-result-object v1

    if-nez v1, :cond_ee

    goto :goto_f1

    :cond_ee
    invoke-virtual {v1, v0}, Lcom/codemao/nemo/view/LoginInputLayout;->setTextWatcher(Landroid/text/TextWatcher;)V

    .line 124
    :goto_f1
    invoke-virtual {p0}, Lcom/codemao/nemo/activity/login/AccountLoginAction;->getSecondLoginLayout()Lcom/codemao/nemo/view/LoginInputLayout;

    move-result-object v1

    if-nez v1, :cond_f8

    goto :goto_fb

    :cond_f8
    invoke-virtual {v1, v0}, Lcom/codemao/nemo/view/LoginInputLayout;->setTextWatcher(Landroid/text/TextWatcher;)V

    :goto_fb
    return-void
.end method

.method public final onMainStart(Lcom/codemao/nemo/event/MainStartEvent;)V
    .registers 2
    .annotation runtime Lorg/greenrobot/eventbus/Subscribe;
    .end annotation

    .line 284
    invoke-virtual {p0}, Lcom/codemao/nemo/activity/login/AccountLoginAction;->getContext()Lcom/codemao/nemo/activity/login/AccountLoginActivityV3;

    move-result-object p1

    if-eqz p1, :cond_9

    invoke-virtual {p1}, Lcom/giu/xzz/BaseActivity;->finish()V

    :cond_9
    return-void
.end method
